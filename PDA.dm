...
/obj/item/device/pda/verb/verb_remove_pen()
	...
	set name = "Remove Pen"
	set src in usr
	var/obj/item/Object = loc
	var/list/contained_item = list(/obj/item/weapon/pen, /obj/item/toy/crayon, /obj/item/weapon/lipstick, /obj/item/device/flashlight/pen)
	...
	if (usr.canUseTopic(src))
		for(i = 1, i <= contained_item.len, i++)
			Object = locate(contained_item[i]) in src	//If items are in the PDA
			if(Object)
				if (istype(loc, /mob))
					var/mob/M = loc
					M.put_in_hands(Object)
					usr << "<span class='notice'>You remove \the [Object] from \the [src].</span>"
					return
				Object.forceMove()
...
/obj/item/device/pda/attackby(obj/item/C, mob/user, params)
	...
	if(istype(C, /obj/item/weapon/cartridge) && !cartridge)
		...
		var/i
		for(i = 1, i < contained_item.len, i++)
			O = locate(contained_item[i]) in src
		if(O)
